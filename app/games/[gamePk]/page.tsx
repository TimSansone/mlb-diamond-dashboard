...
type BattingStats = {
  atBats?: number; runs?: number; hits?: number; rbi?: number; baseOnBalls?: number;
  strikeOuts?: number; homeRuns?: number; doubles?: number; triples?: number; avg?: string;
};
...
function PlayerFeature({ label, person, primary, secondary }: { label: string; person?: Person; primary: string; secondary: string }) {
  return (
    <div className={styles.playerFeature}>
      {person?.id ? <img src={headshot(person.id)} alt={`${person.fullName ?? label} headshot`} width={92} height={92} /> : <span className={styles.featurePlaceholder} />}
      <div><span className={styles.featureLabel}>{label}</span><strong>{person?.fullName ?? "TBD"}</strong><p>{primary}</p><small>{secondary}</small></div>
    </div>
  );
}
...
export default async function GameCenterPage({ params }: { params: Promise<{ gamePk: string }> }) {
  ...
  const scoringPlays = allPlays.filter((play) => play.about?.isScoringPlay); const recentPlays = allPlays.slice(-14).reverse();
      ...
      {(live || currentPlay) && (
        <section className={styles.livePanel}>
          <div className={styles.matchupPlayers}>
            <PlayerFeature label="Current batter" person={batter} primary={`AVG ${batterAverage}`} secondary={batterGameLine(batterStats)} />
            <span className={styles.versus}>VS</span>
            <PlayerFeature label="Current pitcher" person={pitcher} primary={`${pitches} pitches · ${pitchBalls} balls · ${pitchStrikes} strikes`} secondary={`${pitcherStats?.inningsPitched ?? "0.0"} IP · ${pitcherStats?.strikeOuts ?? 0} K · ERA ${pitcherData?.seasonStats?.pitching?.era ?? pitcherStats?.era ?? "—"}`} />
          </div>
          <div className={styles.liveState}>
            <BaseDiamond first={offense?.first} second={offense?.second} third={offense?.third} />
            <div className={styles.counts}><span><b>{line?.balls ?? currentPlay?.count?.balls ?? 0}</b> Balls</span><span><b>{line?.strikes ?? currentPlay?.count?.strikes ?? 0}</b> Strikes</span><span><b>{line?.outs ?? currentPlay?.count?.outs ?? 0}</b> Outs</span></div>
          </div>
