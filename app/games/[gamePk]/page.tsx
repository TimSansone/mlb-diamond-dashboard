...
type TeamBox = TeamBoxscore & {
  teamStats?: { batting?: TeamStats; pitching?: TeamStats };
  pitchers?: number[];
...
const headshot = (playerId?: number) =>
  playerId
    ? `https://img.mlbstatic.com/mlb-photos/image/upload/w_160,q_auto:best,f_auto/v1/people/${playerId}/headshot/67/current`
    : "";
...
function BaseDiamond({ first, second, third }: { first?: Person; second?: Person; third?: Person }) {
  const occupied = [
    first && `first: ${first.fullName}`,
    second && `second: ${second.fullName}`,
    third && `third: ${third.fullName}`,
  ]
    .filter(Boolean)
    .join(", ") || "none";

  return (
    <div className={styles.baseDisplay} aria-label={`Bases occupied: ${occupied}`}>
      ...
      <div className={styles.runnerLabels}>
        <span className={styles.runnerSecond}>{second?.fullName ?? "2nd empty"}</span>
        <span className={styles.runnerThird}>{third?.fullName ?? "3rd empty"}</span>
        <span className={styles.runnerFirst}>{first?.fullName ?? "1st empty"}</span>
      </div>
    </div>
  );
}
...
function PlayerFeature({
  ...
  return (
    <div className={styles.playerFeature}>
      {person?.id ? (
        <img src={headshot(person.id)} alt={`${person.fullName ?? label} headshot`} width={92} height={92} />
      ) : (
        <span className={styles.featurePlaceholder} />
      )}
      <div>
        <span className={styles.featureLabel}>{label}</span>
        <strong>{person?.fullName ?? "TBD"}</strong>
        <p>{primary}</p>
...
export default async function GameCenterPage({ params }: { params: Promise<{ gamePk: string }> }) {
  ...
  const latestCompletePlay = [...allPlays]
    .reverse()
    .find((play) => play.about?.isComplete !== false && Boolean(play.result?.description || play.result?.event));
  ...
  const pitcher = matchup?.pitcher ?? line?.defense?.pitcher;
  const batterData = playerFromBoxes(batter?.id, awayBoxscore, homeBoxscore);
  const pitcherData = playerFromBoxes(pitcher?.id, awayBoxscore, homeBoxscore);
  const batterStats = batterData?.stats?.batting;
  const pitcherStats = pitcherData?.stats?.pitching;
  const pitches = pitcherStats?.numberOfPitches ?? 0;
  const pitchStrikes = pitcherStats?.strikes ?? 0;
  const pitchBalls = Math.max(0, pitches - pitchStrikes);
    ...
    <div className={styles.page}>
        ...
        <section className={styles.livePanel}>
          <div className={styles.matchupPlayers}>
            <PlayerFeature
              ...
              person={batter}
              primary={`AVG ${batterAverage}`}
              secondary={batterGameLine(batterStats)}
            />
            <span className={styles.versus}>VS</span>
            <PlayerFeature
              label="Current pitcher"
              person={pitcher}
              primary={`${pitches} pitches · ${pitchBalls} balls · ${pitchStrikes} strikes`}
              secondary={`${pitcherStats?.inningsPitched ?? "0.0"} IP · ${pitcherStats?.strikeOuts ?? 0} K · ERA ${pitcherData?.seasonStats?.pitching?.era ?? pitcherStats?.era ?? "—"}`}
            />
          </div>
          <div className={styles.liveState}>
            <BaseDiamond first={offense?.first} second={offense?.second} third={offense?.third} />
          ...
          <div className={styles.latestPlay}>
            <span className="eyebrow">Latest completed play</span>
            <p>{latestCompletePlay?.result?.description ?? latestCompletePlay?.result?.event ?? "No completed play is available yet."}</p>
          </div>
        </section>
      )}
      ...
      {(decisions?.winner || decisions?.loser || decisions?.save) && (
        <section className={styles.decisions}>
          <div><span>Winning pitcher</span><strong>{decisions.winner?.fullName ?? "—"}</strong></div>
          <div><span>Losing pitcher</span><strong>{decisions.loser?.fullName ?? "—"}</strong></div>
          <div><span>Save</span><strong>{decisions.save?.fullName ?? "—"}</strong></div>
        </section>
      )}
      ...
      <div className={styles.grid}>
              ...
              {scoringPlays.map((play, index) => (
                <li key={`${play.about?.inning}-${index}`}>
                  <div>
                    ...
                    {play.result?.awayScore !== undefined && <b>{play.result.awayScore}–{play.result.homeScore}</b>}
                  </div>
        ...
        <section className={styles.card}>
          <h2>Play-by-play</h2>
          <InningPlayByPlay plays={allPlays} currentInning={currentInning} />
        </section>
      </div>
    </div>
  );
